...
const DartsRules=require('../../public/darts-rules');
...
function start(game,previous,amount,options,revision,rng=randomInt){
 ...
 else if(game==='darts'){const range=2**32,area=rng(range)/range,angle=rng(range)/range*Math.PI*2;m=DartsRules.outcome(area);r.detail={radius:Math.sqrt(area),angle,rules:DartsRules.version};}
