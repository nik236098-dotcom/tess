...
(function(root){
 ...
 function frame(info,t){
  ...
  const cars=geometry.lanes.map(i=>{
   ...
   // Integrate a smooth velocity ramp, cruising speed, then braking.
   // Even the last car crosses before the .88 braking point.
   const position=q=>{q=clamp(q);const a=.22,b=.12,v=end/(1-a/2-b/2);
    if(q<a){const s=q/a;return v*a*(s*s*s-.5*s*s*s*s);}
    if(q<=1-b)return v*(q-a/2);
    const s=(q-1+b)/b;return v*(1-b-a/2+b*(s-s*s*s+.5*s*s*s*s));
   };
   const distance=position(u),h=.0001;
   const speed=u>0&&u<1?(position(u+h)-position(u-h))/(4*h):0;
   const acceleration=u>0&&u<1?(position(u+h)-2*distance+position(u-h))/(4*h*h):0;
   const scale=1-.4*distance,y=geometry.startY-(geometry.startY-geometry.finishY-geometry.nose*.6)*distance;
   return {speed,acceleration,x:200+(i-1.5)*(83-24*distance),y,scale,distance,noseY:y-geometry.nose*scale,finished:distance>=1,wheel:0};
  });
 ...
 function board(info,animating,side){
  ...
  return `<div class="rc-scene"><div class="rc-heading"><span>${done?'Финиш':animating?'Заезд':'Выберите машину'}</span><b>3.92× <small>· 25%</small></b></div><div class="rc-viewport" data-rc-side="${selected}"><svg class="rc-track rc-fallback" viewBox="0 0 400 410" role="img" aria-label="Четыре машины на трассе. Выбрана машина ${selected+1}."><defs><linearGradient id="rc-asphalt" x2="0" y2="1"><stop stop-color="#1b2039"/><stop offset="1" stop-color="#272c49"/></linearGradient><linearGradient id="rc-curb" x2="1" y2="0"><stop stop-color="#6558b1"/><stop offset=".5" stop-color="#b8acf1"/><stop offset="1" stop-color="#403965"/></linearGradient><pattern id="rc-checks" width="16" height="16" patternUnits="userSpaceOnUse"><rect width="16" height="16" fill="#ece9fc"/><path d="M0 0H8V8H0ZM8 8H16V16H8Z" fill="#292943"/></pattern>${[0,1,2,3].map(n=>`<filter id="rc-paint-${n}" color-interpolation-filters="sRGB"><feColorMatrix type="hueRotate" values="${[0,120,40,195][n]}"/></filter>`).join('')}</defs>
   ...
   <path data-rc-finish="${geometry.finishY}" d="M93 ${geometry.finishY-10}H307L310 ${geometry.finishY}H90Z" fill="url(#rc-checks)" opacity=".95"/>
   <path d="M84 108V80M316 108V80" fill="none" stroke="#9984e7" stroke-width="2"/><path d="M84 80h15v10H84ZM316 80h15v10h-15Z" fill="url(#rc-checks)"/>
   <g class="rc-lights">${[0,1,2].map(i=>`<circle data-rc-light="${i}" cx="381" cy="${283+i*14}" r="4" fill="#3c3455"/>`).join('')}</g>
  ...
  </svg></div><div class="rc-race-status" role="status" aria-live="polite">${animating?'На старт…':done?'Первым финишировал №'+(info.detail.winner+1):'Ваша машина: №'+(selected+1)}</div><div class="rc-order" aria-label="Порядок финиша">${done?info.detail.order.map((n,i)=>`<span style="--car-color:${carColors[n]}" class="${n===selected?'is-selected':''}"><small>${i+1}</small><i></i>№${n+1}</span>`).join(''):geometry.lanes.map(i=>`<span class="rc-order-empty">${i+1}<small>—</small></span>`).join('')}</div></div>`;
 }
 function paint(host,info,t){
  const f=frame(info,t),viewport=host.querySelector('.rc-viewport');
  const rendered=root.RaceRenderer?.render(viewport,{...f,countdown:t>0?f.countdown:0},Number(viewport?.dataset.rcSide)||0,carColors);
  viewport?.classList.toggle('rc-has-canvas',Boolean(rendered));
  if(!rendered)f.cars.forEach((c,i)=>host.querySelector(`[data-rc-car="${i}"]`)?.setAttribute('transform',`translate(${c.x} ${c.y+c.wheel}) scale(${c.scale})`));
