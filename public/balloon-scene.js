...
(function(root){
 ...
 function board(info,animating,locked,coefficients=[],previous=null){
  ...
  <div class="bl-readout"><div class="bl-steps" role="list" aria-label="Шаги и коэффициенты" tabindex="0">${steps}</div><span role="status" aria-live="polite">${animating?'Надуваем шар…':lost?'Шар лопнул · 0,00×':done?'Раунд завершён · '+number(info.multiplier??0)+'×':`Успешных качков: ${step}`}</span></div></div>`;
 }
