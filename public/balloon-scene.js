...
(function(root){
 ...
 function frame(info,previous,t){
  ...
  return {scale:.68+.013*Math.min(25,step),pump:stroke*55,sway:pumping?Math.sin(t*Math.PI*4)*(1-t)*1.8:0,burst:burst&&t>=.66,progress:t===1?1:clamp((t-.66)/.34)};
 }
 function board(info,animating,locked){
  ...
  return `<div class="bl-panel"><svg class="bl-scene" viewBox="0 0 360 390" role="img" aria-label="Воздушный шар с ручным насосом"><defs>
   ...
   <radialGradient id="bl-latex" cx="32%" cy="22%" r="83%"><stop stop-color="#94caff"/><stop offset=".25" stop-color="#398ffc"/><stop offset=".57" stop-color="#0870eb"/><stop offset=".83" stop-color="#0754b7"/><stop offset="1" stop-color="#052960"/></radialGradient>
   ...
   <radialGradient id="bl-shadow"><stop stop-color="#020915" stop-opacity=".9"/><stop offset="1" stop-color="#020915" stop-opacity="0"/></radialGradient>
  </defs><rect width="360" height="390" fill="url(#bl-room)"/><path d="M0 325Q180 300 360 325V390H0Z" fill="#0d2039"/>
  ...
  <g id="bl-balloon"><path d="M131 284C118 267 41 211 34 141C26 67 68 20 129 19C190 18 236 68 227 138C220 204 147 267 131 284Z" fill="url(#bl-latex)" stroke="#56a3f1" stroke-opacity=".6" stroke-width="1.2"/>
  <path d="M47 130C41 83 78 40 110 38C91 62 70 84 63 128Z" fill="url(#bl-shine)" filter="url(#bl-soft)"/><path d="M208 111C222 157 196 204 181 215C191 180 198 149 196 120Z" fill="url(#bl-shine)" filter="url(#bl-soft)" opacity=".55"/>
  <path d="M129 263L131 283 138 267M119 261L131 283" fill="none" stroke="#79bcff" stroke-opacity=".25"/>
  ...
  <g id="bl-handle"><rect x="274" y="146" width="8" height="129" rx="3" fill="url(#bl-chrome)"/><rect x="239" y="138" width="79" height="12" rx="5" fill="url(#bl-chrome)"/><rect x="231" y="133" width="31" height="23" rx="6" fill="url(#bl-grip)" stroke="#6f8096" stroke-width=".6"/><rect x="294" y="133" width="31" height="23" rx="6" fill="url(#bl-grip)" stroke="#6f8096" stroke-width=".6"/></g>
  <path d="M252 344H304L316 352V359H240V352Z" fill="url(#bl-chrome)" stroke="#68809d"/><rect x="259" y="232" width="39" height="114" rx="4" fill="url(#bl-body)" stroke="#6e8aaa" stroke-width=".6"/>
  <rect x="257" y="228" width="43" height="13" rx="4" fill="url(#bl-chrome)"/><ellipse cx="278.5" cy="228" rx="21.5" ry="4" fill="url(#bl-chrome)"/><rect x="257" y="336" width="43" height="10" rx="3" fill="url(#bl-chrome)"/><rect x="249" y="327" width="14" height="10" rx="3" fill="url(#bl-chrome)"/></svg>
  <div class="bl-readout" role="status" aria-live="polite"><b id="bl-value">${animating?'…':((info?.multiplier||1).toFixed(2)).replace('.',',')+'×'}</b><span>${animating?'Надуваем шар…':done?info.last?.safe===false?'Шар лопнул':'Раунд завершён':`Успешных качков: ${info?.step||0}`}</span></div></div>${live?`<button type="button" class="cg-action bl-inflate" data-cg-pick="0" ${locked?'disabled':''}>${animating?'Надуваем…':'Надуть ещё'}</button>`:''}`;
 }
