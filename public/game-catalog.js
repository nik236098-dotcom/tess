...
(function(root){
  ...
  function tile(id,native=false){
    ...
    return `<button type="button" ${native?`id="play-${id}"`:`data-arcade="${id}"`} class="game-tile" aria-label="${text(name)}"><img src="/img/game-cards/${file(id)}.${['cryo','midnight'].includes(id)?'svg':'webp'}" alt="" width="600" height="800" loading="lazy" decoding="async"><span class="game-tile-title${name.length>13?' is-long':''}">${text(name)}</span></button>`;
  }
