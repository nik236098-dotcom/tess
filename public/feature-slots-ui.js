...
const FeatureSlotsUI=(()=>{
 ...
 const sprite=n=>`<span class="fs-symbol" role="img" aria-label="${R.symbols[n].name}" style="--sx:${n%5*25}%;--sy:${n<5?0:100}%"></span>`;
 const symbol=(n,win=false)=>`<div class="ax-cell${win?' is-win':''}${n===9?' is-scatter':''}" data-symbol="${n}">${sprite(n)}</div>`;
 const lockIcon='<svg viewBox="0 0 32 36" aria-hidden="true"><path d="M8 15V9a8 8 0 0 1 16 0v6"/><rect x="3" y="14" width="26" height="20" rx="3"/><path d="M16 21v7"/></svg>';
 ...
 function assets(){
  ...
  const assetGame=game;Promise.all(['scene-v2','symbols-v2'].map(name=>new Promise(resolve=>{const image=new Image();image.onload=()=>resolve(true);image.onerror=()=>resolve(false);image.src='/img/feature-slots/'+game+'/'+name+'.webp';}))).then(result=>{if(assetGame!==game)return;loading=false;ready=result.every(Boolean);failed=!ready;if(active())render(false);});
 }
 function prepare(id){
 ...
 function controls(){
  ...
  const frozen=game==='cryo';
  return `<div class="ax-controls ax-console fs-console${bonus?' is-feature':''}"><div class="ax-console-readout"><span>${bonus?'BONUS WIN':'WIN'}<b>${money(visual?.payout||0)}</b></span></div><div class="ax-console-actions"><div class="fs-stake-controls">${!bonus?`<button type="button" data-ax="minus" class="fs-bet-mod" aria-label="Уменьшить ставку" ${locked||pending||bet===R.stakes[0]?'disabled':''}>−</button>`:''}<button type="button" data-ax="stake" class="ax-console-tile fs-stake" ${locked||pending||bonus?'disabled':''}><small>BET</small><strong>${money(bonus?info.unitBet:bet)}</strong></button>${!bonus?`<button type="button" data-ax="plus" class="fs-bet-mod" aria-label="Увеличить ставку" ${locked||pending||bet===R.stakes.at(-1)?'disabled':''}>+</button>`:''}</div><button type="button" class="ax-spin${a.animating?' is-spinning':''}" data-ax="spin" aria-label="${pending?'Получить сохранённую выплату':bonus?'Продолжить бесплатные вращения':'Вращать за '+money(bet)}" ${locked||loading?'disabled':''}>${icon}</button>${bonus?`<button type="button" data-ax="pause" class="fs-buy fs-auto" ${celebrating||entering?'disabled':''}><strong>${running?'Ⅱ AUTO':'▷ AUTO'}</strong><small>${running?'Пауза':'Продолжить'}</small></button>`:`<button type="button" data-ax="buy" class="fs-buy" ${locked||pending||!ready?'disabled':''}><small>BONUS BUY</small><strong>${money(price)}</strong></button>`}</div><div class="ax-console-status" role="status">${status}</div><div class="ax-utilities"><button type="button" data-ax="menu" aria-label="Настройки слота"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 3 1-1h4l1 3 3 1 3 3-1 3 1 3-3 3-3 1-1 3h-4l-1-3-3-1-3-3 1-3-1-3 3-3 3-1Z"/><circle cx="12" cy="12" r="3.5"/></svg>${frozen?'':'SETTINGS'}</button><button type="button" data-ax="turbo" aria-pressed="${turbo}" ${locked?'disabled':''}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14 2-10 12h7l-1 8 10-13h-7Z"/></svg>TURBO<i></i></button></div></div>`;
 ...
 function render(board=true){
  ...
  const plaque=game==='cryo'?`<div class="fs-feature-plaque"><b>DEEP FREEZE</b><span>${inBonus?'FREE SPINS <strong>'+bonus.remaining+'</strong>':'3 SCATTER · FREE SPINS'}</span></div>`:inBonus?`<div class="fs-feature-plaque fs-train-plaque"><b>VAULT HEIST</b><span>FREE SPINS <strong>${bonus.remaining}</strong></span></div>`:'';
  const progression=game==='midnight'?`<div class="fs-vault-track" aria-label="Ключи ${bonus?.keys||0} из 9, множитель ${shownMultiplier}"><div>${[3,6,9].map((n,i)=>`<span class="fs-lock${(bonus?.keys||0)>=n?' is-open':''}${(bonus?.keys||0)<n&&(bonus?.keys||0)>=n-3?' is-current':''}">${lockIcon}<b>${[2,3,5][i]}×</b></span>`).join('')}</div><small>VAULT EXPRESS${inBonus?' · '+(bonus?.keys||0)+'/9':''}</small></div>`:'<div class="fs-frozen-caption">WILDS STAY FROZEN</div>';
  $('ag-stage').innerHTML=`<div class="ax-scene fs-scene"><div class="ax-hero"><h1 class="fs-accessible-title">${R.title}</h1>${plaque}</div><div class="ax-window"><i class="fs-corner tl"></i><i class="fs-corner tr"></i><i class="fs-corner bl"></i><i class="fs-corner br"></i><div class="ax-grid" aria-label="Барабаны ${R.title}">${reels}</div></div>${progression}<p class="ax-scene-note">${note}</p></div>`;
 ...
 function show(kind){
  ...
  else body=`<h2>Символы и выплаты</h2><p>За 3 / 4 / 5 подряд слева направо.<br>Множители ниже — от ставки одной линии.</p><div class="ax-payments">${R.symbols.slice(0,9).map(s=>`<div><div class="fs-pay-symbol">${sprite(Number(s.id))}</div><span>${s.name}</span><b>${s.pay.join(' / ')}×</b></div>`).join('')}</div><h3>20 выигрышных линий</h3><div class="ax-payline-guide">${R.lines.map((line,i)=>`<div><small>${i+1}</small><svg viewBox="0 0 100 60" aria-label="Линия ${i+1}"><polyline points="${line.map((row,col)=>`${col*20+10},${row*20+10}`).join(' ')}"/></svg></div>`).join('')}</div><p>${$('ag-rules-text').textContent}</p>`;
