'use strict';
/* Native SVG silhouettes avoid sprite cutouts and device-specific emoji hands. */
(function(root){
 const shapes=[
 ...
 function hand(index,prefix='hand',tone='violet'){
  ...
  return `<svg class="duel-hand-art" viewBox="0 0 100 116" aria-hidden="true"><defs><linearGradient id="${prefix}-skin" x2=".75" y2="1"><stop stop-color="#fff9f0"/><stop offset=".48" stop-color="#f5e9df"/><stop offset="1" stop-color="#dbc5c9"/></linearGradient><linearGradient id="${prefix}-cuff" x2=".3" y2="1"><stop stop-color="${base}"/><stop offset="1" stop-color="${shade}"/></linearGradient></defs><path d="${shapes[i]}" fill="#695178" opacity=".3" transform="translate(1 3)"/><path d="${shapes[i]}" fill="url(#${prefix}-skin)" stroke="#c8b0bd" stroke-width="1.4" stroke-linejoin="round"/><path d="${lines}" fill="none" stroke="#b994a3" stroke-width="1.5" stroke-linecap="round"/><path d="M33 87Q49 96 70 87L71 103Q51 111 32 103Z" fill="url(#${prefix}-cuff)" stroke="${shade}" stroke-width="1"/><path d="M36 92Q51 98 66 92" fill="none" stroke="#fff" opacity=".4" stroke-width="2" stroke-linecap="round"/></svg>`;
 }
