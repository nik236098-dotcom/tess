...
test('catalog includes exactly the 19 approved additions and excludes baccarat',()=>{assert.equal(ids.length,19);assert.ok(!ids.includes('baccarat'));});
...
test('scratch hides unopened symbols, rejects duplicate reveal, pays after exactly nine cells',()=>{
 let r=game.start('scratch',game.initial(),100,{},0,()=>9999%1); // zero roll: losing ticket
 assert.deepEqual(game.publicState('scratch',r).grid,Array(9).fill(null));
 for(let i=0;i<9;i++){const old=r;r=game.actGame('scratch',r,'ag_pick',i,r.revision);assert.equal(game.publicState('scratch',r).grid.filter(Boolean).length,i+1);assert.throws(()=>game.actGame('scratch',r,'ag_pick',i,r.revision));assert.equal(old.revealed.length,i);}
 assert.equal(r.payout,0);
});
test('weighted prize tables total 100% and return 98% before cent rounding',()=>{
 for(const id of ['scratch','cases','fishing']){const table=catalog.distribution(id);assert.equal(table.reduce((s,[w])=>s+w,0),10000);assert.ok(Math.abs(table.reduce((s,[w,m])=>s+w*m,0)/10000-.98)<1e-12);}
});
...
test('all 19 games persist to disk and reopening never recredits their result',t=>{
