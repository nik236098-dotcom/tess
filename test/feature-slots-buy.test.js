...
const rules=require('../public/feature-slots-rules').cryo;
async function harness(balance=10000){
 ...
 const state={balance,isAdmin:false,ag:{game:'cryo',pending:null,animating:false,options:{},info:{phase:'done',settled:true,detail:{grid:Array(15).fill(0),lines:[],win:0},bonus:{remaining:0,multiplier:1}}}};
 const c=vm.createContext({state,$,FeatureSlotRules:require('../public/feature-slots-rules'),FeatureSlotAudio:{theme(){},unlock(){},stop(){},settings:()=>({muted:true}),setMode(){}},AbyssFX:{clear(){}},GameResult:{hide(){}},money:n=>'$'+(n/100).toFixed(2),agLocked:()=>!!state.ag.pending||state.ag.animating,
  ...
  agRequest:action=>requests.push({action,amount:$('ag-amount').value,options:{...state.ag.options.cryo}})});
 vm.runInContext(fs.readFileSync('public/feature-slots-ui.js','utf8')+'\nglobalThis.ui=FeatureSlotsUI;',c);c.ui.prepare('cryo');await Promise.resolve();await Promise.resolve();
