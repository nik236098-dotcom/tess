...
test('the solid ball rolls according to its travelled distance and remains supported',()=>{
 let previous=0;for(let i=0;i<=100;i++){const f=s.frame(strike,i/100);assert.ok(Math.abs(f.roll+f.travel/s.R)<1e-10);assert.ok(f.travel>=previous);previous=f.travel;assert.equal(f.y,s.R);assert.ok(f.orientation.every(Number.isFinite));}
});
...
test('a zero result rolls into the gutter and misses every pin',()=>{
 ...
 for(let i=0;i<=100;i++){const f=s.frame(info,i/100);assert.ok(f.pins.every(p=>p.fall===0));if(f.z<-4.9){assert.ok(Math.abs(f.x-1.82)<1e-12);for(const [x,z]of s.pins)assert.ok(Math.hypot(f.x-x,f.z-z)>s.R+.25);}}
});
test('the final count matches the server; no pin sinks into the lane while falling',()=>{
 for(const mask of [0,1,341,512,1023]){
  const info={detail:{fallen:Array.from({length:10},(_,i)=>Boolean(mask&(1<<i)))}};
  assert.equal(s.frame(info,1).pins.filter(p=>p.active).length,info.detail.fallen.filter(Boolean).length);
 }
...
test('the renderer waits for its texture, reuses the canvas and recovers after context loss',()=>{
 ...
 vm.runInContext(fs.readFileSync('public/bowling-physics.js','utf8'),ctx);
