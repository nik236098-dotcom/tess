...
(async () => {
  ...
  for (const provider of payments.providers.values()) {
    try { await provider.getMe(); console.log(`✅ ${provider.title}: токен принят · ${provider.currency}${provider.apiVersion ? ' · API ' + provider.apiVersion : ''}`); }
    catch (error) { const message = String(error.message).split(provider.token).join('[скрыто]'); console.error(`❌ ${provider.title}: ${message}`); process.exitCode = 1; }
  }
