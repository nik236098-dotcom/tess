...
Current home revision: transaction preview is replaced by Live, showing up to eight real recent wins from the existing five-second rooms polling feed. Rows show player display name, game artwork, settled payout and payout/stake multiplier; unavailable ratios are shown as a dash (e.g. table records). Feed retains the newest 12 wins in `recent-wins.json` beside the account data, writes each new result through an atomic rename, and loads it on startup. Disk-disabled test instances keep it in memory. The client shows loading until the server supplies the feed. Account transaction history remains in profile. No New Games section was added. Loading uses a verified alpha WebP cutout, without CSS background blending or edge masking.

Wallpaper and hero now follow the first soft-violet reference: subtle corner illumination and fine grain behind the lobby, quiet center behind Live, a large crocodile and one cube in the hero. The two generated WebP assets contain no UI text; white NVUTI and the small new-game caption remain HTML, and the entire banner retains keyboard and pointer activation.
