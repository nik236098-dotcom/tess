...
Approved smoked-glass revision: beveled wallet and action buttons, glass history panel with compact horizontal empty state, floating bottom dock with localized active highlight. Hero illustration was recreated from the approved reference without text or controls; its head extends above the illustrated panel. Popular cards select artwork regions directly from the approved reference via CSS background positioning. Typography and controls remain live HTML. Telegram native top bar is outside webpage control. Visual rendering in Telegram remains unverified; no pixel-identical claim.
